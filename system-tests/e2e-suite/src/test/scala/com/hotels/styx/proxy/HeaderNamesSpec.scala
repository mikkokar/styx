...
class HeaderNamesSpec  extends FunSpec
  ...
  describe("Proxied responses have expected headers added") {
    it("should add response headers") {
      ...
      assertThat(resp.header(HeaderKey.headerKey("styx-info-foo")), matches(matchesRegex("^noJvmRouteSet;[0-9a-f-]+$")))
      assertThat(resp.header(HeaderKey.headerKey("origin-id-bar")), matches(is("generic-app-01")))
