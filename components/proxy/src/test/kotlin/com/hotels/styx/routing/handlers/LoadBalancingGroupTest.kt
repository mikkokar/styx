...
class LoadBalancingGroupTest : FeatureSpec() {
    ...
    init {
        feature("Load Balancing") {
            ...
            routeDb.watch().waitUntil { it.entrySet().size == 3 }
            ...
            scenario("Load balances initial origins") {
            ...
            scenario("... and detects new origins") {
                ...
                routeDb.watch().waitUntil { it.entrySet().size == 6 }
            ...
            scenario("... and emits NoAvailableHostsException when load balancing group is empty") {
                ...
                routeDb.watch().waitUntil { it.entrySet().isEmpty() }
        ...
        feature("Metrics") {
            // TODO: HostProxy could also record the Origins.Response.Status metric
            // metricsRegistry.counter("origins.response.status." + response.status().code()).inc();

            // NOTE: requests.cancelled() has been moved to HostProxy.
            // TODO: Ensure requests.cancelled() is not called twice. Once from HostProxy,
            //       and then from LoadBalancingGroup/BackendServiceClient
        }
